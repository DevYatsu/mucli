...
use crate::antivirus::responses::{AnalysisIdResponse, AnalysisReportResponse, ErrorResponse};
...
use self::responses::AnalysisReportData;
...
pub async fn antivirus_command(sub_matches: &ArgMatches) {
    if let Some(filepath) = sub_matches.get_one::<PathBuf>("FILEPATH") {
        ...
        match is_dangerous(filepath).await {
            Ok(b) => match b {
                true => {
                    print_info!("File is certainly dangerous!");
            ...
            Err(e) => print_err!("(processing error): {}", e),
        };
    }
}

async fn is_dangerous(file_path: &PathBuf) -> Result<bool, AntivirusError> {
